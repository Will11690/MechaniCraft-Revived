...
public class BasicInfuserBlockEntity extends BaseMachineBlockEntity implements IInfuserSync {

    private static final int BASE_CAPACITY = 38_400;
    private static final int BASE_TRANSFER = 128;
    private static final int FE_PER_TICK = 16;
    ...
    public BasicInfuserBlockEntity(BlockPos pos, BlockState state) {

        super(MechaniCraftBlockEntities.BasicInfuserBE.get(), pos, state, true, BASE_CAPACITY, BASE_TRANSFER, true, 0);
    ...
    public void tickServer() {
        ...
        boolean wasLit = getBlockState().getValue(BasicInfuser.LIT);
        ...
        if (match == null) {
            ...
            if (!canAcceptOutput(output)) {
                ...
                if (progress != 0) progress = 0;
            } else if (energy == null || energy.getEnergyStored() < FE_PER_TICK) {

                if (progress > 0) progress = Math.max(0, progress - 2);
            } else {

                energy.consumeEnergy(FE_PER_TICK);
        ...
        if (wasLit != workingThisTick) {
            ...
            level.setBlockAndUpdate(worldPosition, getBlockState().setValue(BasicInfuser.LIT, workingThisTick));
        }
